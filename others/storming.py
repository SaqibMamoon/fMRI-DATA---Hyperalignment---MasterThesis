...
for i in np.arange(len(ds_train)):
    notan = np.zeros(ds_train[i].shape[0])
    zero = np.zeros(ds_train[i].shape[0])
    for raw in np.arange(ds_train[i].shape[0]):
        notan[raw] = np.sum(1*np.isnan(ds_train[i].samples.T))
        zero[raw] = np.sum(1*(ds_train[i].samples.T[:,raw] == 0))
    zeros.append(zero)
    nans.append(notan)
