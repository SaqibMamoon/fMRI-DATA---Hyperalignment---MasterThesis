...
nm_subj = np.loadtxt('/home/andreabertana/Projects/list_subject.txt')
...
for i in np.arange(10,55,5):
    ...
    for j in np.arange(10,55,10):
        
        #take i subjects for training the classifier where i=  10,20,30,40,50
        ds_classifier = ds_all[0:7]
        ...
        hypmaps_hyp = hyper(ds_train_classif)
        ds_hyper_classif = [ hypmaps_hyp[i].forward(sd) for i, sd in enumerate(ds_train_classif)]
        ...
        #project the second run of test group subjects (ds_test_t) in commonspace
        hypmaps_t = hyper(ds_test_t)
        ds_hyper_test = [ hypmaps_t[i].forward(sd) for i, sd in enumerate(ds_test_t)]
        ...
        acc = np.mean(predictions == ds_hyper_test.sa.targets)
        ...
        res = (res_ws, res_hyp)
        path_save = path_save + '_anova_svm' + i +'.txt'
        
        np.savetxt(path_save,res)
