...
for name in nm_subj:
    ...
    path_mask = path + 'atlas_masks/' + name + '_anova1000.nii.gz'
...
ds_test = [sd[sd.sa.chunks == 2,:] for sd in ds_thirty]
...
hypmaps_hyp = hyper(ds_test)
ds_hyper_classif = [ hypmaps_hyp[k].forward(sd) for k, sd in enumerate(ds_test)]
...
res_cv = cv(ds_hyper_classif_stack)
...
#ds_hyper_test = [ hypmaps_t[h].forward(sd) for h, sd in enumerate(ds_test_t)]
#
##zscore timeseries in commonspace for better performance
...
#j= str(j.astype('int'))
#path_realcat = './Projects/realcat'+ i_save + '_' + j + '.txt'
#np.savetxt(path_realcat , ds_hyper_test.sa.targets, fmt='%s') 
#path_prediction = './Projects/prediction'+ i_save + '_' + j +'.txt'
#np.savetxt(path_prediction,prediction, fmt='%s')
#path_save =  './Projects/acc_' + i_save + '_' + j + '.txt' 
