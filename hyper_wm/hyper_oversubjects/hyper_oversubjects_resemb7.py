...
nm_subj = np.loadtxt('/home/andreabertana/Projects/list_subject65_resemb7.txt')
...
for i in np.arange(10,55,5):
    ...
    for j in np.arange(10,35,10):
        ...
        j= str(j.astype('int'))
        path_realcat = './Projects/results/realcat'+ i_save + '_' + j + '_7.txt'
        np.savetxt(path_realcat , ds_hyper_test.sa.targets, fmt='%s') 
        path_prediction = './Projects/results/prediction'+ i_save + '_' + j +'_7.txt'
        np.savetxt(path_prediction,prediction, fmt='%s')
        path_save =  './Projects/results/acc_' + i_save + '_' + j + '_7.txt' 
