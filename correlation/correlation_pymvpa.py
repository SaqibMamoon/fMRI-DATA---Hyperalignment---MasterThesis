...
for name in nm_subj:
    ...
    path_mc = path + 'mc/vt/' + name + '_mc_vtharv.nii.gz'
    #path_det = path + 'detrend/vt/' + name + '_det_mc_vt.nii.gz'
    #path_det = path + 'detrend/harvard/' + name + '_det_mc.nii.gz'
    #path_smo = path + 'smoothing/det/' + name + '_smooth_det.nii.gz'
    #path for vt mask
    #path_vt = path + 'atlas_masks/merge_harv_dx.nii.gz'
    path_vt = path + 'atlas_masks/nnzero/' + name + '_vtharv_nnzero_dx.nii.gz'
    #path_save = path + 'mc/correlation/dx/' + name + '_mc_best500_dx.nii.gz'
    path_save = path + 'mc/correlation/harvard/dx/' + name + '_det_best500_dx.nii.gz'
    path_save2 = path + 'mc/correlation/harvard/dx/' + name + '_det_best300_dx.nii.gz'
    #path for labels
    final = fmri_dataset(samples = path_mc, mask = path_vt)
...
for name in nm_subj:
    ...
    scores.sort()
    save = path + 'mc/correlation/harvard/dx/scores/' + name + 'scores'
...
for name in nm_subj:
    ...
    #path_smo = path + 'smoothing/det/' + name + '_smooth_det.nii.gz'
    #path for vt mask
    path_vt = path + 'atlas_masks/nnzero/' + name + '_vtharv_nnzero_sx.nii.gz'
    #path_vt = path + 'vt_mask/' + name + '_nnzero_restricted_sx.nii.gz'
    #path_save = path + 'mc/correlation/dx/' + name + '_mc_best500_dx.nii.gz'
    path_save = path + 'mc/correlation/harvard/sx/' + name + '_det_best500_sx.nii.gz'
    path_save2 = path + 'mc/correlation/harvard/sx/' + name + '_det_best300_sx.nii.gz'
    #path for labels
    final = fmri_dataset(samples = path_mc, mask = path_vt)
    #Create list og subjects
    ds_all.append(final)
...
for name in nm_subj:
    ...
    scores = ds_all[sd].fa['bpcs'].value
    scores.sort()
    save = path + 'mc/correlation/harvard/sx/scores/' + name + 'scores'
    np.savetxt(save,scores)
    sd = sd +1  
